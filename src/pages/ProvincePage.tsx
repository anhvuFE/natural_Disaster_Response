...
export default function ProvincePage() {
  ...
  useEffect(() => {
    fetchProvinces()
      ...
      .catch(() => toast({ title: "Không tải được tỉnh", variant: "destructive" }));
  }, [fetchProvinces, setCurrentProvinceBySlug, slug, toast]);

  useEffect(() => {
    fetchDisasterTypes().catch(() =>
      toast({ title: "Không tải được loại thiên tai", variant: "destructive" })
    );
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        toast({ title: "Không tải được dữ liệu", description: String(err), variant: "destructive" });
      } finally {
        setLoading(false);
      }
    };
    load();
  }, [activeDisaster, currentProvince, getContacts, getGuide, getLatestAlert, getShelters, toast]);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-2xl border border-white/70 bg-white/90 p-4 shadow-lg shadow-slate-900/5 backdrop-blur">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Loại thiên tai</p>
            <h2 className="text-xl font-semibold text-slate-900">{disasterName || "Chọn loại thiên tai"}</h2>
          </div>
      ...
      {loading && (
        <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white/90 px-4 py-3 text-sm text-slate-700 shadow-sm backdrop-blur">
          ...
          Đang tải dữ liệu cho {disasterName || activeDisaster}
        </div>
      )}
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="space-y-4 lg:col-span-2">
            <ChecklistSection
              ...
              items={guide.before}
            />
            <TipsSection title="Trong lúc xảy ra" sections={guide.during} />
            <TipsSection title="Sau khi thiên tai" sections={guide.after} />
          </div>
      ...
      {!loading && !guide && (
        <div className="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-6 text-center text-sm text-slate-600">
          Không có dữ liệu hướng dẫn cho lựa chọn này.
        </div>
      )}
    </div>
  );
